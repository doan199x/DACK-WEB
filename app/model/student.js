...
module.exports = {
    ...
    getWatchList: async (studentID, courseID) => {
        const sql = `SELECT* from WatchList where studentID =${studentID} and courseID = ${courseID}`;
        const result = await db.load(sql);
        return result;
    },
    insertWatchList: async (studentID, courseID) => {
        const sql = `INSERT into WatchList (studentID,courseID) values(${studentID},${courseID})`;
        const result = await db.load(sql);
        return result;
    },
}; 